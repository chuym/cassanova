...
Query.prototype.WHERE = function(key){
    if(!!key) {
        Query.utils.verifySchemaKey(key, value, this, true);
...
/**
 * Chainable, A shortcut, it adds "WHERE" and "IN" into the CQL statement.
 * WHERE userid IN (123, 456)
 */
Query.prototype.WHERE_IN = function(key, options){
    var inOpt = [];
    ...
    this.cql_str += "WHERE " + key + " IN (" + inOpt.join(", ") + ")";
